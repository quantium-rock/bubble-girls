import img1 from '../../images/0.png';
import img2 from '../../images/1.png';
import img3 from '../../images/1.png';
import img4 from '../../images/2.png';

export const About = {
    id: '1',
    lightBg: false,
    lightText: true,
    lightTextDesc: true,
    topLine: 'ABOUT US',
    headline: 'The revolution of NFTs, Music & Metaverse ',
    description: "BubbleGirls 1st generation 750 unique NFT that will give access to a Metaverse party-room.",
    description1: "* NFT: BubbleGirls collection will be launched in Solana as one of the most efficents blockchain.",
    description2: "* MUSIC: BubbleGirls collection will be launched in Solana as one of the most efficents blockchain.",
    description3: "* METAVERSE: BubbleGirls collection will be launched in Solana as one of the most efficents blockchain.",
    buttonLabel: 'Get started',
    imgStart: false,
    img: img1,
    alt: '',
    dark: true,
    primary: true,
    darkText: false
};

export const Rarities = {
    id: '2',
    lightBg: true,
    lightText: false,
    lightTextDesc: true,
    topLine: 'collections',
    headline: '10 Different Collections',
    description: 'eeeeo',
    buttonLabel: 'Get started',
    imgStart: true,
    img: img2,
    alt: '',
    dark: true,
    primary: true,
    darkText: false
};

export const Gamearn = {
    id: '3',
    lightBg: false,
    lightText: true,
    lightTextDesc: true,
    topLine: 'RARITIES',
    headline: 'Rarities provide value to your NFTs',
    description: "In #0friendsclub we count with 4 levels of value that will determinate your Rarity Level.",
    buttonLabel: 'Get started',
    imgStart: false,
    img: img3,
    alt: '',
    dark: true,
    primary: true,
    darkText: false
};

export const Roadmap = {
    id: '4',
    lightBg: true,
    lightText: false,
    lightTextDesc: false,
    topLine: 'Premium Bank',
    headline: 'Unlimited Transactions with zero fees',
    description: 'Get accesss to our exclusive app that allows you to send unlimited transaction with 0 fees. ',
    buttonLabel: 'Get started',
    imgStart: true,
    img: img4,
    alt: '',
    dark: false,
    primary: false,
    darkText: false
};